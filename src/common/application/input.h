...
namespace Engine
{
	...
	struct InputService : public Engine::Service
	{
		ENGINE_DECLARE_SERVICE( InputService );
		...
		bool								is_mouse_down( MouseButtons buttons );
		bool								is_mouse_clicked( MouseButtons buttons );
		bool								is_mouse_released( MouseButtons buttons );
		bool								is_mouse_dragging( MouseButtons buttons );

		void								update( f32 delta );

		//void								debug_ui();

		void								new_frame();			// Called before messahe handling
		...
		// Create Method used to create the actual input.
		...
		Engine::StringBuffer				string_buffer;
