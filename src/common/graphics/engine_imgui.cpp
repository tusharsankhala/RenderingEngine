...
#include "graphics/engine_imgui.h"
...
namespace Engine
{
    ...
    // Graphics Data
    static Engine::TextureHandle g_font_texture;
    static Engine::PipelineHandle g_imgui_pipeline;
    static Engine::BufferHandle g_vb, g_ib;
    static Engine::BufferHandle g_ui_cb;
    static Engine::DescriptorSetLayoutHandle g_descriptor_set_layout;
    static Engine::DescriptorSetHandle g_ui_descriptor_set;  // Font descriptor set

    static uint32_t g_vb_size = 665536, g_ib_size = 665536;

    Engine::FlatHashMap<Engine::ResourceHandle, Engine::ResourceHandle> g_texture_to_descriptor_set;
    ...
    void ImGuiService::shutdown() {
        ...
        while (it.is_valid()) {
            Engine::ResourceHandle handle = g_texture_to_descriptor_set.get(it);
    ...
    void ImGuiService::render( Engine::CommandBuffer& commands ) {
            ...
            for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
            {
                ...
                else
                {
                    ...
                    if (clip_rect.x < fb_width && clip_rect.y < fb_height && clip_rect.z >= 0.0f && clip_rect.w >= 0.0f)
                    {
                        ...
                        commands.draw_indexed( Engine::TopologyType::Triangle, pcmd->ElemCount, 1, index_buffer_offset + pcmd->IdxOffset, vtx_buffer_offset + pcmd->VtxOffset, new_texture.index);
                    }
                }
    ...
    void ImGuiService::remove_cached_texture( Engine::TextureHandle& texture )
    {
        FlatHashMapIterator it = g_texture_to_descriptor_set.find(texture.index);
        if (it.is_valid()) {

            // Destroy descriptor set
            Engine::DescriptorSetHandle descriptor_set{ g_texture_to_descriptor_set.get(it) };
