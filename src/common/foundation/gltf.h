...
#define InjectDefault3DModel() \
	if ( Engine::file_exists( kDefault3DModel ) ) {\
		argc = 2;\
	...
	namespace glTF
	{
		...
		struct AnimationSampler
		{
			...
			/*	LINEAR: The animated values are linearly interpolated between keyframes.When targeting a rotation,
				spherical linear interpolation(slerp)** SHOULD** be used to interpolate quaternions.The float of output elements** MUST** equal the float of input elements.
				...
				The float of output elements **MUST** equal three times the float of input elements.
		...
		struct MeshPrimitive
		{
			...
			i32								mode;
			// u32 targets_count;
			// object* targets; // TODO(macro) This is a JSON object.
		};
