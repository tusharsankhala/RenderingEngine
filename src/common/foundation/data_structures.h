...
namespace Engine
{
	...
	struct ResourcePool
	{
		void						init( Allocator* allocator, u32 pool_size, u32 resoirce_size );
		void						shutdown();

		u32							obtain_resource();					// Return the index to the resource.
	...
	template <typename T>
	T* ResourcePoolTyped<T>::get( u32 index )
	{
		return ( T* )ResourcePool::access_resource(index);
	}

	template <typename T>
	const T* ResourcePoolTyped<T>::get( u32 index ) const
	{
