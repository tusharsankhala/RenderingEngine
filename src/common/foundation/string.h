...
namespace Engine
{
	...
	struct StringBuffer
	{
		...
		char*								append_use( const StringView& text );			// Append and return the pointer to the start. Used for string mostly.
		char*								append_use_substring(const char* string, u32 start_index, u32 end_index); // Append a substring to the passed string.
	...
	struct StringArray
	{
		...
		FlatHashMap<u64, u32>*				string_to_index;		// Note: Trying to avoid bringing the hap map header.
		FlatHashMapIterator*				strings_iterator;		// Note: Trying to avoid bringing the hap map header.
